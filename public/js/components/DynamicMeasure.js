...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(
    ...
    showLabelSegment: function(layer, index, fromIndex, points) {
        ...
        var created = this.showLabel(layer, index, fromIndex,
        ...
        if (maxSegments !== null) {
            if (created) {
                var hide = (layer.features.length - maxSegments) -1;
                if (hide >= 0) {
                    layer.features[hide].style = {display: 'none'};
                    layer.drawFeature(layer.features[hide]);
                }
            } else {
                var show = (layer.features.length - maxSegments);
                if (show >= 0) {
                    delete layer.features[show].style;
                    layer.drawFeature(layer.features[show]);
                }
            }
        }
        return created;
