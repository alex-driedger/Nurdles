define([
    ...
    var InfoView = Backbone.View.extend({
        ...
        getReports: function () {
            returnStatement = [];
            for (i in this.collection[2].attributes)
            {
                var date = new Date(this.collection[2].attributes[i].created)
                returnStatement.push({html: ("Report Date : " + date.getDate() + "-" + date.getMonth() + "-" + date.getFullYear() + " at " + date.getHours() + ":" + date.getMinutes()), id: this.collection[2].attributes[i]._id})
            }
            return returnStatement
        },
        render: function () {
            ...
            } else
            {
                ...
                this.$el.html(_.template(infoTemplate, {average: average, conditions: conditions, high: high, low: low, beachInfo: attributes, ratesHTML: this.getRates(), surveysHTML: this.getSurveys(), reportsHTML: this.getReports()}));
                return this;
