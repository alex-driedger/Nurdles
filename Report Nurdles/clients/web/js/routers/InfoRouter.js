define([
    ...
    'views/InfoView',
    'authentication'
], function ($, _, Backbone, BeachSurveyModel, BeachRateModel, BeachReportModel, BeachModel, InfoView, Authentication) {
                ...
                beachRates.fetch({
                    success: function (collection, response, options) {
                        ...
                        beachSurveys.fetch({
                            success: function (collection, response, options) {
                                collections.push(collection)
                                beachReports = new BeachReportModel.Collection([], {
                                    beachID: id
                                });
                            beachReports.fetch({
                            success: function (collection, response, options) {
                                ...
                                beaches.fetch({
                                    success: function (collection, response, options) {
                                        collections.push(collection)
                                        beaches = new BeachModel.Collection([], {
                                            lat: collections[3].models[0].attributes.lat,
                                            lon: collections[3].models[0].attributes.lon,
                                            request: "forecast"
                                        });
                                        beaches.fetch({
                                            success: function (collection, response, options) {
